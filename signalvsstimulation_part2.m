function final = signalvsstimulation_part2()
...
%}
    ...
    % Part 2: Coregister full spectrum with stim intensity  
      ...
      numSegments = floor(height(final) / samplesPerSegment);

      gammaData = table();  % initialize

      for s = 1:numSegments
          idxStart = (s-1)*samplesPerSegment + 1;
          idxEnd = s*samplesPerSegment;

          seg = final(idxStart:idxEnd, :);

          % Ignore segment if non-continuous
          if any(diff(seg.Time) > seconds(1/fs)*1.5)
              continue;  % skip if large time gap
          end

          % Left Channel
          [pxxL, fL] = pwelch(double(seg.LeftRaw), welchWindow, overlap, nfft, fs);
          avgStimL = mean(seg.LeftStim_mA);
      ...
      gammaPowersL = zeros(height(gammaData),1);
      gammaPowersR = zeros(height(gammaData),1);

      for i = 1:height(gammaData)
          fL = gammaData.FreqL{i};
          pxxL = gammaData.PowerL{i};
          fR = gammaData.FreqR{i};
          pxxR = gammaData.PowerR{i};

          % Extract gamma band power as area under curve
          gammaPowersL(i) = bandpower(pxxL, fL, gammaRange, 'psd');
          gammaPowersR(i) = bandpower(pxxR, fR, gammaRange, 'psd');
      end

      % Plot
      figure('Name','Gamma Power vs Stimulation');

      subplot(2,1,1);
      yyaxis left;
      plot(gammaPowersL, '-o'); ylabel('Left Gamma Power');
      yyaxis right;
      plot(gammaData.AvgStimL, '-x'); ylabel('Left Avg Stim (mA)');
      xlabel('Segment'); title('Left: Gamma Power vs Stimulation'); grid on;

      subplot(2,1,2);
      yyaxis left;
      plot(gammaPowersR, '-o'); ylabel('Right Gamma Power');
      yyaxis right;
      plot(gammaData.AvgStimR, '-x'); ylabel('Right Avg Stim (mA)');
      xlabel('Segment'); title('Right: Gamma Power vs Stimulation'); grid on;

      assignin('base','gammaData',gammaData);
      disp('Step 2 complete: gammaData table created with frequency spectrum and stim per segment.');
end
