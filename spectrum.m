...
selected_stim_rate = comboStimRates(selection);
...
if isempty(signal_cells)
    error('No valid signal data found for selected combination.');
end

% *** CRITICAL FIX: Respect actual recording timeframes ***
% Instead of creating artificial sequential timestamps, we process each 
% recording segment separately and then combine results appropriately

fprintf('Processing %d separate recording segments...\n', length(signal_cells));

% Calculate actual total recording time (not sequential time)
...
for i = 1:length(signal_cells)
    ...
    % Calculate actual duration of this segment
    nSamples = length(signal);
    duration_sec = nSamples * sample_interval_ms / 1000;
    segment_durations(i) = duration_sec;
    
    fprintf('  Segment %d: %.1f seconds (%d samples)\n', i, duration_sec, nSamples);
end

total_actual_duration = sum(segment_durations);
fprintf('Total actual recording duration: %.1f seconds (%.1f minutes)\n', ...
    total_actual_duration, total_actual_duration/60);

% Process each segment for spectral analysis
epoch_duration_sec = 10; % FIXED: Use reasonable epoch length (was 100!)
welch_window_sec = 1;
welch_overlap = 0.5;
...
for seg_idx = 1:length(signal_cells)
    ...
    fprintf('  Segment %d: %.1f sec -> %d epochs\n', seg_idx, segment_duration_sec, num_epochs_this_segment);
...
fprintf('Total valid epochs extracted: %d\n', total_epochs);

if total_epochs == 0
    error('No valid epochs found for analysis. Try shorter epoch duration.');
end

% Calculate spectra for all epochs
fprintf('Computing power spectra...\n');

% Get frequency vector from first epoch
...
selectedFiles = unique(all_BrainSenseTimeDomain.SourceFile(selectedIndices));
title_str = sprintf('%s %s - Power Spectrum\n%s | %.1f Hz | %d epochs (%.1f min total)', ...
...
xlim([0, min(100, max(f))]);
grid on; grid minor;

legend({'Mean Spectrum', '±1 STD', sprintf('Gamma (%.1f-%.1f Hz)', gamma_band(1), gamma_band(2))}, ...
...
spectrum_results.frequencies = f;
...
spectrum_results.num_epochs = total_epochs;
spectrum_results.total_duration_minutes = total_actual_duration/60;
spectrum_results.source_files = selectedFiles;
spectrum_results.epoch_duration_sec = epoch_duration_sec;

assignin('base', 'spectrum_results', spectrum_results);

fprintf('\n=== COMPLETE ===\n');
fprintf('Results saved as ''spectrum_results''\n');
